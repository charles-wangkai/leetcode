...
public class Solution {
	public boolean wordPatternMatch(String pattern, String str) {
		...
		return isMatched(pattern, str, new ArrayList<Character>(letter2count.keySet()), letter2count,
				new HashMap<Character, Integer>(), 0, pattern.length(), str.length());
	}

	boolean isMatched(String pattern, String str, List<Character> letters, Map<Character, Integer> letter2count,
			Map<Character, Integer> letter2length, int index, int remainPatternLength, int remainStrLength) {
		...
		for (int length = beginLength; remainPatternLength - count <= remainStrLength - length * count; length++) {
			letter2length.put(letter, length);

			if (isMatched(pattern, str, letters, letter2count, letter2length, index + 1, remainPatternLength - count,
