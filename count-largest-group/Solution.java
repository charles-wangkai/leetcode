...
class Solution {
    public int countLargestGroup(int n) {
        ...
        int maxCount = digitSumToCount.values().stream().mapToInt(x -> x).max().getAsInt();

        return (int) digitSumToCount.keySet().stream().filter(digitSum -> digitSumToCount.get(digitSum) == maxCount)
                .count();
    }
}