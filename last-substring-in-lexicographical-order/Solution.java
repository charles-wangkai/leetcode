	...
	int[] buildSuffixArray(String s) {
		...
		for (int k = 4; k < 2 * n; k *= 2) {
			...
			for (int i = 0; i < nextSuffixes.length; i++) {
				...
				nextSuffixes[i].nextRank = (nextIndex < n) ? nextSuffixes[indexToSuffixIndex[nextIndex]].rank : -1;
			}
...
class Suffix implements Comparable<Suffix> {
	...
	public int compareTo(Suffix other) {
		return (rank != other.rank) ? Integer.compare(rank, other.rank) : Integer.compare(nextRank, other.nextRank);
	}
}