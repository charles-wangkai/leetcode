...
FROM (
    SELECT
        ...
        DATEDIFF(LEAST(s.period_end, y.end_date), GREATEST(s.period_start, y.begin_date)) + 1 AS day_num
    FROM Sales s
    JOIN Product p
    ON p.product_id = s.product_id
    JOIN (
        SELECT "2018" AS report_year, '2018-01-01' AS begin_date, '2018-12-31' AS end_date
        UNION
