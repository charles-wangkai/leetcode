...
public class CoinPath {
		...
		for (int i = 1; i < length; i++) {
			...
			if (A[i] >= 0) {
				for (int j = 1; j <= B && j <= i; j++) {
					if (costs[i - j] >= 0 && (costs[i] < 0 || costs[i - j] + A[i] < costs[i]
							|| (costs[i - j] + A[i] == costs[i] && isLess(concat(paths[i - j], i + 1), paths[i])))) {
	...
	List<Integer> concat(List<Integer> path, int sequence) {
		List<Integer> result = new ArrayList<Integer>(path);
		result.add(sequence);
		return result;
	}
