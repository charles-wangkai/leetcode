...
class Solution {
    ...
    Element search(List<Integer>[] adjLists, List<Boolean> hasApple, boolean[] visited, int v) {
        visited[v] = true;
        ...
        for (int adj : adjLists[v]) {
            if (!visited[adj]) {
                Element subResult = search(adjLists, hasApple, visited, adj);
        ...
        return new Element(time, foundApple);
    }
}
...
class Element {
    ...
    Element(int time, boolean foundApple) {
        this.time = time;
