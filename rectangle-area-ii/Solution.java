...
public class Solution {
	...
	public int rectangleArea(int[][] rectangles) {
		int[] xValues = Arrays.stream(rectangles).flatMapToInt(rectangle -> IntStream.of(rectangle[0], rectangle[2]))
				.distinct().sorted().toArray();
		int[] yValues = Arrays.stream(rectangles).flatMapToInt(rectangle -> IntStream.of(rectangle[1], rectangle[3]))
				.distinct().sorted().toArray();
		...
		for (int i = 0; i < fills.length; i++) {
			for (int j = 0; j < fills[0].length; j++) {
				if (fills[i][j]) {
					area = addMod(area, multiplyMod(xValues[i + 1] - xValues[i], yValues[j + 1] - yValues[j]));
				}
			}
		}
		return area;
