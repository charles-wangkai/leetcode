	...
	public List<String> wordSubsets(String[] A, String[] B) {
		...
		for (String wordB : B) {
			...
			for (char letter : letterToCountB.keySet()) {
				letterToMinCount.put(letter,
						Math.max(letterToMinCount.getOrDefault(letter, 0), letterToCountB.get(letter)));
			}
		}

		return Arrays.stream(A).filter(wordA -> isUniversal(letterToMinCount, wordA)).collect(Collectors.toList());
	}
