...
public class Solution {
  public List<String> wordsAbbreviation(List<String> dict) {
    ...
    Map<LastAndLength, List<Integer>> lastAndLength2indices = new HashMap<>();
    for (int i = 0; i < dict.size(); i++) {
      String word = dict.get(i);

      LastAndLength lastAndLength =
          new LastAndLength(word.charAt(word.length() - 1), word.length());

      if (!lastAndLength2indices.containsKey(lastAndLength)) {
        lastAndLength2indices.put(lastAndLength, new ArrayList<>());
      }
  ...
  void divide(List<String> result, List<String> dict, int prefixLength, List<Integer> indexList) {
    ...
    Map<Character, List<Integer>> ch2indices = new HashMap<>();
    for (int index : indexList) {
      char ch = dict.get(index).charAt(prefixLength);

      if (!ch2indices.containsKey(ch)) {
        ch2indices.put(ch, new ArrayList<>());
      }
