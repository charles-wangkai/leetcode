...
class Solution {
  ...
  boolean isWin(char[][] grid, char target) {
    return IntStream.range(0, SIZE)
            .anyMatch(r -> IntStream.range(0, SIZE).allMatch(c -> grid[r][c] == target))
        || IntStream.range(0, SIZE)
            .anyMatch(c -> IntStream.range(0, SIZE).allMatch(r -> grid[r][c] == target))
  ...
  boolean isFinished(char[][] grid) {
    return IntStream.range(0, SIZE)
        .allMatch(r -> IntStream.range(0, SIZE).allMatch(c -> grid[r][c] != 0));
  }
}
