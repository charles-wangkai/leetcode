...
public class Solution {
	...
	Map<Character, List<Character>> buildAdjacentsList(String[] words) {
		...
		for (int i = 0; i < words.length - 1; i++) {
			...
			int diffIndex = 0;
			while (diffIndex < word1.length() && diffIndex < word2.length()
			...
			if (diffIndex < word1.length()) {
				if (diffIndex == word2.length()) {
					...
					letter2adjacents.get(word1.charAt(diffIndex)).add(
							word2.charAt(diffIndex));
				}
			}
		}
	...
	Map<Character, List<Character>> initLetter2adjacents(String[] words) {
		Map<Character, List<Character>> letter2adjacents = new HashMap<Character, List<Character>>();
		for (String word : words) {
			for (int i = 0; i < word.length(); i++) {
				char letter = word.charAt(i);
				if (!letter2adjacents.containsKey(letter)) {
					letter2adjacents.put(letter, new ArrayList<Character>());
				}
			}
		}
		return letter2adjacents;
	...
	String topologicalSort(Map<Character, List<Character>> letter2adjacents) {
		...
		Map<Character, Boolean> letter2visited = new HashMap<Character, Boolean>();
		...
		for (char letter : letter2degree.keySet()) {
			if (!letter2visited.get(letter) && letter2degree.get(letter) == 0) {
				dfs(letter2adjacents, letter2visited, letter2degree, order,
						letter);
			}
		}
	...
	Map<Character, Integer> buildLetter2degree(
			Map<Character, List<Character>> letter2adjacents) {
		Map<Character, Integer> letter2degree = new HashMap<Character, Integer>();
	...
	void dfs(Map<Character, List<Character>> letter2adjacents,
			Map<Character, Boolean> letter2visited,
			Map<Character, Integer> letter2degree, StringBuilder order,
			char letter) {
