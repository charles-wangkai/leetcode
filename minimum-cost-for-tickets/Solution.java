public class Solution {
	public int mincostTickets(int[] days, int[] costs) {
		...
		for (int i = 1; i < minCosts.length; i++) {
			minCosts[i] = minCosts[findLastLength(days, i, 1)] + costs[0];
			minCosts[i] = Math.min(minCosts[i], minCosts[findLastLength(days, i, 7)] + costs[1]);
			minCosts[i] = Math.min(minCosts[i], minCosts[findLastLength(days, i, 30)] + costs[2]);
		}
