...
public class Solution {
	...
	boolean search(int[] nums, int subsetSum, boolean[] used, int startIndex, int remainSubset, int remainSum) {
		if (startIndex < 0) {
			return false;
		}
		if (remainSubset == 0) {
		...
		for (int i = startIndex; i >= 0; i--) {
			if (!used[i] && nums[i] <= remainSum) {
				...
				if (nums[i] == remainSum) {
					...
					if (search(nums, subsetSum, used, i - 1, remainSubset, remainSum - nums[i])) {
						return true;
					}
				}
