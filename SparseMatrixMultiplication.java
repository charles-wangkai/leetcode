...
public class SparseMatrixMultiplication {
			...
			for (int colA : sparseMatrixA.get(rowA).keySet()) {
				if (sparseMatrixB.containsKey(colA)) {
					for (int colB : sparseMatrixB.get(colA).keySet()) {
						result[rowA][colB] += sparseMatrixA.get(rowA).get(colA) * sparseMatrixB.get(colA).get(colB);
					}
				}
			}
		}
	...
	Map<Integer, Map<Integer, Integer>> buildSparseMatrix(int[][] m) {
		Map<Integer, Map<Integer, Integer>> sparseMatrix = new HashMap<Integer, Map<Integer, Integer>>();
		for (int row = 0; row < m.length; row++) {
			for (int col = 0; col < m[0].length; col++) {
				if (m[row][col] != 0) {
					if (!sparseMatrix.containsKey(row)) {
						sparseMatrix.put(row, new HashMap<Integer, Integer>());
					}
