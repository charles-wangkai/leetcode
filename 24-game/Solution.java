...
public class Solution {
  static final Operation[] OPERATIONS = {
    ...
    (x, y) ->
        new Rational(
            x.numerator * y.denominator - y.numerator * x.denominator,
            x.denominator * y.denominator),
    (x, y) -> new Rational(x.numerator * y.numerator, x.denominator * y.denominator),
    (x, y) -> {
      if (y.numerator == 0) {
  ...
  public boolean judgePoint24(int[] nums) {
    return search(Arrays.stream(nums).mapToObj(num -> new Rational(num, 1)).toList());
  }
    ...
    for (int i = 0; i < numbers.size(); i++) {
      for (int j = 0; j < numbers.size(); j++) {
        ...
        for (Operation operation : OPERATIONS) {
          List<Rational> nextNumbers = new ArrayList<>();
