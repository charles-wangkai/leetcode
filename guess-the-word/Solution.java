...
public class Solution {
	public void findSecretWord(String[] wordlist, Master master) {
		...
		while (true) {
			...
			candidates = candidates.stream().filter(candidate -> computeScore(candidate, word) == score)
	...
	int computeScore(String candidate, String word) {
		return (int) IntStream.range(0, candidate.length()).filter(i -> candidate.charAt(i) == word.charAt(i)).count();
	}
}
