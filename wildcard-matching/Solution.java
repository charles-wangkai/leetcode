...
public class Solution {
  Map<Pair, Boolean> history = new HashMap<>();
  int maxStarIndex = -1;
  ...
  boolean isMatch(String s, int indexS, String p, int indexP, int nonStarLength) {
    ...
    if (chP == '*') {
      ...
      if (isMatch(s, indexS, p, indexP + 1, nonStarLength)) {
        result = true;
      } else if (maxStarIndex == indexP && isMatch(s, indexS + 1, p, indexP, nonStarLength)) {
        result = true;
      } else {
        result = false;
      }
