...
public class NumberOfDistinctIslands_II {
...
class Island {
	static final Transformation[] TRANSFORMATIONS = { (maxR, maxC, land) -> land,
			...
			(maxR, maxC, land) -> new Point(maxR - land.r, land.c), (maxR, maxC, land) -> new Point(land.c, land.r),
...
interface Transformation {
	...
	default Island transform(Island island) {
		...
		return new Island(island.lands.stream().map(land -> transform(maxR, maxC, land)).collect(Collectors.toList()));
	}
}
