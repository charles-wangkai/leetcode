...
public class Solution {
  public String countOfAtoms(String formula) {
    ...
    return element2count.keySet().stream()
        .sorted()
        .map(
            element ->
                element + (element2count.get(element) == 1 ? "" : element2count.get(element)))
        .collect(Collectors.joining());
  }

  Map<String, Integer> parse(String s) {
    Map<String, Integer> element2count = new HashMap<>();
    int index = 0;
  ...
  Map<String, Integer> add(
      Map<String, Integer> element2count1, Map<String, Integer> element2count2) {
