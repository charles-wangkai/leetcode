		...
		for (int size = 2; size <= length; ++size) {
			for (int beginIndex = 0; beginIndex + size <= length; ++beginIndex) {
				...
				if (s.charAt(beginIndex) == s.charAt(endIndex)) {
					stepNums[beginIndex][endIndex] = (size == 2) ? 0 : stepNums[beginIndex + 1][endIndex - 1];
				} else {
					stepNums[beginIndex][endIndex] = Math.min(stepNums[beginIndex + 1][endIndex],
							stepNums[beginIndex][endIndex - 1]) + 1;
				}
			}
		}
