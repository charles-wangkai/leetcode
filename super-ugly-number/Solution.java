...
public class Solution {
  public int nthSuperUglyNumber(int n, int[] primes) {
    PriorityQueue<UglyNumberAndPrimeIndex> pq =
        new PriorityQueue<>((ni1, ni2) -> Long.compare(ni1.uglyNumber, ni2.uglyNumber));
    ...
    for (int i = 0; i < n; i++) {
      ...
      for (int j = head.primeIndex;
          j < primes.length
              && !(i + 1 + pq.size() >= n && head.uglyNumber * primes[j] >= maxUglyNumber);
          j++) {
