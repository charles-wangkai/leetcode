...
public class RaceCar {
	public int racecar(int target) {
		Map<State, Integer> state2length = new HashMap<State, Integer>();
		...
		while (true) {
			...
			for (State nextState : new State[] { new State(head.position + head.speed, head.speed * 2),
					new State(head.position, -Integer.signum(head.speed)) }) {
				if (nextState.position == target) {
					return nextLength;
				}

				if (nextState.position > 0 && nextState.position < (1 << 14) && !state2length.containsKey(nextState)) {
