...
public class Solution {
  ...
  public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
    code2money = new HashMap<>();
    return search(price, special, needs, 0);
  }

  int search(
      List<Integer> price, List<List<Integer>> special, List<Integer> needs, int specialIndex) {
    ...
    if (specialIndex == special.size()) {
      ...
      while (true) {
        money =
            Math.min(
                money,
                oneSpecial.get(oneSpecial.size() - 1) * specialNum
        ...
        IntStream.range(0, needs.size())
            .forEach(i -> needs.set(i, needs.get(i) - oneSpecial.get(i)));
        specialNum++;
      }
