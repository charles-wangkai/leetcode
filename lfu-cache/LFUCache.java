...
public class LFUCache {
  ...
  LinkList<Integer> freqs = new LinkList<>();
  Map<Integer, LinkList<Integer>> freq2keys = new HashMap<>();

  Map<Integer, Node<Integer>> key2node = new HashMap<>();
  Map<Integer, Node<Integer>> freq2node = new HashMap<>();
  ...
  public int get(int key) {
    ...
    if (!freq2keys.containsKey(newFreq)) {
      freq2keys.put(newFreq, new LinkList<>());

      Node<Integer> node = new Node<>(newFreq);
