...
public class Solution {
	public int minStickers(String[] stickers, String target) {
		...
		List<Map<Character, Integer>> stickersLetter2count = new ArrayList<Map<Character, Integer>>();
		...
		Set<Character> stickersLetters = new HashSet<Character>();
		...
		Set<Map<Character, Integer>> letter2countSet = Collections.singleton(new HashMap<Character, Integer>());
		for (int step = 1;; step++) {
			Set<Map<Character, Integer>> nextLetter2countSet = new HashSet<Map<Character, Integer>>();

			for (Map<Character, Integer> letter2count : letter2countSet) {
				for (Map<Character, Integer> stickerLetter2count : stickersLetter2count) {
					Map<Character, Integer> nextLetter2count = add(letter2count, stickerLetter2count,
							targetLetter2count);
	...
	boolean isBetter(Map<Character, Integer> letter2countFrom, Map<Character, Integer> letter2countTo) {
		for (char letter : letter2countTo.keySet()) {
			if (!letter2countFrom.containsKey(letter) || letter2countFrom.get(letter) < letter2countTo.get(letter)) {
				return false;
			}
		}
		return true;
	...
	Map<Character, Integer> add(Map<Character, Integer> letter2count, Map<Character, Integer> stickerLetter2count,
			Map<Character, Integer> targetLetter2count) {
		Map<Character, Integer> result = new HashMap<Character, Integer>(letter2count);
		for (Entry<Character, Integer> entry : stickerLetter2count.entrySet()) {
			char letter = entry.getKey();
			int count = entry.getValue();

			result.put(letter, Math.min(result.getOrDefault(letter, 0) + count, targetLetter2count.get(letter)));
		}
		return result;
	}
