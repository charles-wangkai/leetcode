...
public class Solution {
  public int getImportance(List<Employee> employees, int id) {
    Map<Integer, Employee> id2employee =
  ...
  int search(Map<Integer, Employee> id2employee, int id) {
    return id2employee.get(id).importance
        + id2employee.get(id).subordinates.stream()
            .mapToInt(subordinate -> search(id2employee, subordinate))
            .sum();
  }
}
