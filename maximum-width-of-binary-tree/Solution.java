...
class Solution {
  public int widthOfBinaryTree(TreeNode root) {
    ...
    return positionLists.stream()
        .mapToInt(
            positionList -> positionList.get(positionList.size() - 1) - positionList.get(0) + 1)
        .max()
        .getAsInt();
  }
  ...
  void search(List<List<Integer>> positionLists, TreeNode node, int depth, int position) {
    ...
    search(positionLists, node.left, depth + 1, position * 2 + 1);
    search(positionLists, node.right, depth + 1, position * 2 + 2);
  }
}
