...
public class Solution {
  Map<Integer, BigInteger> length2countCache = new HashMap<>();

  public int strobogrammaticInRange(String low, String high) {
    return Math.max(
        0,
        countStrobogrammatic(high, false, false)
            .subtract(countStrobogrammatic(decrease(low, false), false, false))
            .intValue());
  }

  BigInteger countStrobogrammatic(String limit, boolean maintainLength, boolean allowLeadingZero) {
    if (limit.isEmpty()) {
  ...
  BigInteger countUpTo(String limit, char digit) {
    return (Integer.parseInt(limit) >= digit - '0') ? BigInteger.ONE : BigInteger.ZERO;
  }
  ...
  BigInteger countUpTo(String limit, char head, char tail) {
    ...
    if (first > head) {
      ...
      if (bound != null && bound.charAt(0) == head) {
        return countStrobogrammatic(bound.substring(1, bound.length() - 1), true, true);
      }
    }
  ...
  BigInteger countStrobogrammatic(int length, boolean allowLeadingZero) {
    ...
    if (length == 0) {
      ...
      count =
          countStrobogrammatic(length - 2, true)
              .multiply(new BigInteger(allowLeadingZero ? "5" : "4"));
    }
  ...
  String decrease(String number, boolean maintainLength) {
    ...
    if (maintainLength) {
      if (result.compareTo(BigInteger.ZERO) < 0) {
        ...
        return String.format(String.format("%%0%dd", number.length()), result);
      }
    } else {
