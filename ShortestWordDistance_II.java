...
public class ShortestWordDistance_II {
	private Map<String, List<Integer>> word2indices = new HashMap<String, List<Integer>>();
	private Map<Set<String>, Integer> history = new HashMap<Set<String>, Integer>();

	public ShortestWordDistance_II(String[] words) {
		for (int i = 0; i < words.length; i++) {
			if (!word2indices.containsKey(words[i])) {
				word2indices.put(words[i], new ArrayList<Integer>());
			}
	...
	public int shortest(String word1, String word2) {
		...
		while (i1 != indices1.size() || i2 != indices2.size()) {
			if (i2 == indices2.size()
					|| (i1 != indices1.size() && indices1.get(i1) < indices2
							.get(i2))) {
				if (i2 > 0) {
					minDistance = Math.min(minDistance, indices1.get(i1)
							- indices2.get(i2 - 1));
				}
				i1++;
			} else {
				if (i1 > 0) {
					minDistance = Math.min(minDistance, indices2.get(i2)
							- indices1.get(i1 - 1));
				}
				i2++;
			}
