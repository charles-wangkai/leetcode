...
public class Solution {
  public List<String> basicCalculatorIV(String expression, String[] evalvars, int[] evalints) {
    Map<String, Integer> variable2value = new HashMap<>();
    ...
    return result.values().stream()
        .sorted(
            (term1, term2) -> {
              if (term1.freeVariables.size() != term2.freeVariables.size()) {
                return -Integer.compare(term1.freeVariables.size(), term2.freeVariables.size());
              }

              for (int i = 0; ; i++) {
                if (!term1.freeVariables.get(i).equals(term2.freeVariables.get(i))) {
                  return term1.freeVariables.get(i).compareTo(term2.freeVariables.get(i));
                }
              }
            })
        .map(Term::toString)
      ...
      if (token.equals("+") || token.equals("-")) {
        ...
        part =
            Collections.singletonMap(
                "", new Term(Collections.emptyList(), token.equals("+") ? 1 : -1));
  ...
  Map<String, Term> multiply(Term term1, Map<String, Term> part2) {
    ...
    for (Term term2 : part2.values()) {
      List<String> freeVariables = new ArrayList<>(term1.freeVariables);
  ...
  Map<String, Term> readPart(
      String s, String token, Expression expr, Map<String, Integer> variable2value) {
    Map<String, Term> part = new HashMap<>();
    if (token.equals("(")) {
